...
- **GUI-first**, a rich click-driven editor. No fighting with config files.
- **HUD image engine**: draw images and GIFs directly on the player's screen.
...
- **Image as a gate**, the `image` action shows the picture and **defers the rest of the list until it ends**. Everything after an `image` action is effectively the "on-finish" sequence.
...
- **High-resolution mode**, large images dropped into the `hd/` subfolder are automatically **sliced into a tile grid** for crisp detail.
...
> **BETA**, this integration is still being tested. It works, but expect rough edges.
